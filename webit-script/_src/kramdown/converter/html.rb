  ...
  module Converter
    ...
    class Html < Base
      ...
      def convert_header(el, indent)
        ...
        @navs.push(nav)
        format_as_block_html("h#{level}", attr, inner_data, indent)
      end
      ...
      def convert_root(el, indent)
        ...
        "<div class=\"ws-sidenav\"><ul class=\"nav\">#{print_nav()}</ul></div><div class=\"ws-doc-body\" >#{result}</div>"
      end
      
      def print_nav()
        navs = @navs
        cur_level = navs[0]['level']
        text = ""
        while !navs.empty?
          item = navs.shift
          if cur_level == item['level']
            text += "<li><a href=\"##{item['id']}\">#{item['text']}</a></li>"
          end
        end
        text
      end
